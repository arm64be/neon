...
local function tool_args_preview(args)
  if args == nil then
    return "{}"
  end

  local ok, encoded = pcall(neon.json.encode, args)
  if not ok then
    return "{}"
  end

  if #encoded > 120 then
    return encoded:sub(1, 117) .. "..."
  end
...
local function resolve_tool_calls(state, messages, message)
  ...
  for _, tool_call in ipairs(tool_calls) do
    ...
    print(("tool> %s %s"):format(call_name, tool_args_preview(args)))
